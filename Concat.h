Automat Concat(int id1, int id2, bool console = true) {
    Automat* aut1 = Automats[id1];
    Automat* aut2 = Automats[id2];
    int new_states = aut1->states + aut2->states;
    int new_start = aut1->start;

    std::vector<int> new_finals;
    if(std::find(aut2->finals.begin(), aut2->finals.end() ,aut2->start) != aut2->finals.end())
    ...
    for(auto state : aut1->finals) {
        for(auto edge : aut2->automat[aut2->start]) {
            new_automat[state].push_back({edge.toVertex+aut1->states, edge.label});
        }
    }
